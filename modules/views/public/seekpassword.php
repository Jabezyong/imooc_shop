    ...
    <a class="brand" href="<?php echo yii\helpers\Url::to(['/index/index']) ?>"></a>
        ...
        <div class="span4 box">
            <div class="content-wrap">
                <p>慕课商城 - Forgot Password</p>
                <?php if (Yii::$app->session->hasFlash('info')) {
                    echo Yii::$app->session->getFlash('info');
                } ?>
                <?php echo $form->field($model, 'adminuser')->textInput(["class" => "span12", "placeholder" => "userid"]); ?>
                <?php echo $form->field($model, 'adminemail')->textInput(["class" => "span12", "placeholder" => "useremail"]); ?>
                <a href="<?php echo yii\helpers\Url::to(['public/login']); ?>" class="forgot">Return to Login</a>
                <?php echo Html::submitButton('Seek Password', ["class" => "btn-glow primary login"]); ?>
            </div>
        </div>
        <?php ActiveForm::end(); ?>
    </div>
